...
class ChatbotModel:
    def __init__(self):
        ...
        self.df = pd.read_json('C:/Users/Koptorinw/Documents/GitHub/TPR/Senior_code/chatbot/data/web_scraping.json')
    ...
    def process_text_input(self, next_location, text_input):
        ...
        if len(f_location) != 0:
            for location in f_location:
                ...
                for ans_place in answer['place_name']:
                    location_list.append(ans_place)

        elif len(p_location) != 0:
            for location in p_location:
                similar_location = pd.concat([similar_location, self.df[self.df['place_name'].str.contains(re.escape(location), na=False)]], ignore_index=True)
                if "type" in predict_label:
                    if not self.df[self.df['place_name'].str.contains(re.escape(location), na=False)].empty:
                        similar_location = self.df[self.df['place_name'].str.contains(re.escape(location), na=False)].sample(n=5).reset_index(drop=True)
                answer = similar_location[similar_location.columns[matching_column]]  # Get the columns that match the specified labels
                for ans_place in answer['place_name']:
                    location_list.append(ans_place)
        ...
        elif next_location and predict_label:
        ...
        return answer, location_list

    def format_result(self, answer):
        result = ""
        value_empty = {"address": "ที่อยู่",
        ...
        for index, row in answer.iterrows():
            ...
            for col, value in zip(answer.columns, row):
                if value == "":
                    indented_values.append(f"ฉันไม่มีข้อมูลเกี่ยวกับ{value_empty[col]}ของสถานที่นี้")
                else:
...
while True:
    ...
    answer, location_list = chatbot_model.process_text_input(next_location, text_input)
